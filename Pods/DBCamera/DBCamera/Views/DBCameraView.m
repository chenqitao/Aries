...
#define previewFrame (CGRect){ 0, 70, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height - 180 }
...
- (UIButton *) photoLibraryButton
{
    if ( !_photoLibraryButton ) {//看相片
        ...
        [_photoLibraryButton setFrame:(CGRect){ 25,  CGRectGetMidY(self.bottomContainerBar.bounds) - 20, 44, 44 }];
...
{
    if ( !_triggerButton ) {//照相
        _triggerButton = [UIButton buttonWithType:UIButtonTypeCustom];
//        [_triggerButton setBackgroundColor:self.tintColor];
        [_triggerButton setImage:[UIImage imageNamed:@"icon_photo_photo page.png"] forState:UIControlStateNormal];
...
- (UIButton *) cameraButton
{
    if ( !_cameraButton ) {//翻转
        ...
        [_cameraButton setAutoresizingMask:UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin];
